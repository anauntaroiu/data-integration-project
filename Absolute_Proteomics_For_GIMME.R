...
# Replace missing data points with '-1' placeholders
for (i in 1:dim(Absolute_Data)[2]){ # Loop through each column in data matrix
  
  for (j in 1:dim(Absolute_Data)[1]) { # Loop through each row in data matrix
    
    if (is.na(Absolute_Data[j,i])){ # Check if expression value was measured 
      Absolute_Data[j,i] <- -1 # Add placeholder
    }
  }
}
...
# Create random data to match absolute data
###########################################
...
for (j in 2:16){ # Loop through each replicate
  ...
  measured_values <- as.numeric(Absolute_Data[grep('-1', Absolute_Data[,j], invert = T), j])

  # Save medians of extracted data for GIMME integration
  medians[(j-1), 1] <- median(as.numeric(Absolute_Data[grep('-1', Absolute_Data[,j], invert = T), j]))
...
matching_expression <- matrix(data = 0, nrow = length(Reactions), ncol = 15)
...
for (x in 2:16){ # Run loop for each replicate and sample of data matrix
  ...
  for (i in 1:dim(genes_and_reactions)[1]){ # Run loop for each reaction in model
    ...
    if (genes_and_reactions[i,2] == -1){ # If no gene ID exists for reaction, put placeholder
      ...
      hit <- grep(genes_and_reactions[i,2], Absolute_Data[,1]) # Determine whether expression was measured for a reaction
      
      if (length(hit) == 0 || Absolute_Data[hit[1],x] == -1){ # If expression was not measured for a reaction
        
        matching_expression[i, (x-1)] <- -1 # Put placeholder for expression value
        random_samples[i, index:(index + 9)] <- -1 # Put placeholder in random samples data as well
        
      }else{ # Expression values exists
        matching_expression[i, (x-1)] <- as.numeric(Absolute_Data[hit[1],x]) # Save expression value
      }
    }
  }
...
setwd("C:/Users/Ana/Documents/MATLAB/Capstone/Absolute Proteomics")

write.csv(matching_expression, file = "Absolute Proteomics For GIMME.csv", row.names = F)
write.csv(medians, file = "Medians For Absolute Proteomics.csv", row.names = F)
write.csv(random_samples, file = "Random Samples For Absolute Proteomics.csv", row.names = F)
write.csv(medians_random, file = "Random Sample Medians For Absolute Proteomics.csv", row.names = F)
